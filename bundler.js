...
class TsLoader extends Loader {
	...
	load(fileChanged=null) {
		if(fileChanged && !minimatch(fileChanged.filepath, '**/*.{ts,tsx,svg,html}')) {
		...
		if(this.#decorators) {
			result = babel.transformSync(result, {
				...
				minified: this.#minify, compact: this.#minify,
			}).code;
		}
...
module.exports = {Bundler, FontLoader, ImgLoader, PugLoader, TsLoader, ScssLoader};
