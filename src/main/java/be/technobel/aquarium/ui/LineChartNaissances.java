...
public class LineChartNaissances extends BorderPane {
    ...
    public LineChartNaissances(Statistiques stats) {
        this.stats = stats;
        ...
        lineChart.setTitle("Nombre de naissances en fonction du nombre de tours");
    ...
    public void refresh() {
        ...
        dataAlgues.clear();
        dataPoissons.clear();
        stats.getNaissancesAlgues().forEach(p -> dataAlgues.add(new XYChart.Data<>(p.key(), p.value())));
        stats.getNaissancesPoissons().forEach(p -> dataPoissons.add(new XYChart.Data<>(p.key(), p.value())));
